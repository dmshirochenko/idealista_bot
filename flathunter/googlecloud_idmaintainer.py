...
class GoogleCloudIdMaintainer:
    ...
    def get_exposes_since(self, min_datetime):
        ...
        res = []
        for doc in self.database.collection("exposes").order_by("created_sort").limit(10000).stream():
    ...
    def get_recent_exposes(self, count, filter_set=None):
        ...
        res = []
        for doc in self.database.collection("exposes").order_by("created_sort").limit(100).stream():
            expose = doc.to_dict()
    ...
    def get_last_run_time(self):
        ...
        for doc in (
            self.database.collection("executions")
