...
class Crawler:
    ...
    user_agent_rotator = UserAgent(
        popularity=[Popularity.COMMON._value_], hardware_types=[HardwareType.COMPUTER._value_]
    )
        ...
        while not resolved:
            ...
            for proxy in proxies_list:
                ...
                try:
                    ...
                    resp = requests.get(
                        url, headers=self.HEADERS, proxies={"http": proxy, "https": proxy}, timeout=(20, 0.5)
                    )
    ...
    def resolvegeetest(self, driver, api_key: str = None):
        ...
        while solved == False:
            ...
            postrequest = f"http://2captcha.com/in.php?key={api_key}&method=geetest&gt={gt}&challenge={challenge}&api_server=api.geetest.com&pageurl={urllib.parse.quote_plus(driver.current_url)}"
